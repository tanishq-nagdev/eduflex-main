...
function Grades() {
  const { grades, loading } = useApp();
  const [sortBy, setSortBy] = useState("date"); // date, course, grade, score
  ...
  const filteredAndSortedGrades = grades
    .filter(grade => {
      ...
      const matchesGrade = filterGrade === "all" || 
        ...
        (filterGrade === "below-C" && !["A+", "A", "A-", "B+", "B", "B-", "C+", "C", "C-"].includes(grade.grade));
    ...
    .sort((a, b) => {
      ...
      switch(sortBy) {
        ...
        case "grade":
          ...
          const gradeValues = {"A+": 97, "A": 93, "A-": 90, "B+": 87, "B": 83, "B-": 80, "C+": 77, "C": 73, "C-": 70, "D": 65, "F": 50};
          aValue = gradeValues[a.grade] || 0;
          bValue = gradeValues[b.grade] || 0;
          break;
        case "score":
          aValue = parseInt(a.score.replace('%', ''));
          bValue = parseInt(b.score.replace('%', ''));
          break;
        default:
          aValue = a.date;
          bValue = b.date;
  ...
  const calculateStats = () => {
    if (filteredAndSortedGrades.length === 0) return { average: 0, highest: 0, lowest: 0, totalAssignments: 0 };
    
    const scores = filteredAndSortedGrades.map(grade => parseInt(grade.score.replace('%', '')));
    const average = Math.round(scores.reduce((sum, score) => sum + score, 0) / scores.length);
    const highest = Math.max(...scores);
    const lowest = Math.min(...scores);
    
    return { average, highest, lowest, totalAssignments: filteredAndSortedGrades.length };
  };
